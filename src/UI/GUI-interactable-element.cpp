...
// InputTextField class implementation

// Protected funtions

void InputTextField::click()
{
    if (clicked) return;
...
void InputTextField::draw(sf::RenderTarget& target, sf::RenderStates state) const
{
    target.draw(this->rect,state);
    target.draw(this->text,state);
...
void InputTextField::updateValue(char32_t chr)
{
    // Backspace
    if (chr == 8) {
...
void InputTextField::setFilter(inputFieldFilterCb_t filter)
{
    this->filter = filter;
}

sf::String InputTextField::getValue()
{
    return this->value;
}

bool InputTextField::containPos(sf::Vector2f pos)
{
    return this->rect.getGlobalBounds().contains(pos);
}

void InputTextField::handleEvent(const std::optional<sf::Event>& e)
{
    if (const sf::Event::MouseMoved *mouseMoved = e->getIf<sf::Event::MouseMoved>()) {
        sf::Vector2f mousePos = this->target_ptr->mapPixelToCoords(mouseMoved->position);
        if (containPos(mousePos)) this->hoverIn();
